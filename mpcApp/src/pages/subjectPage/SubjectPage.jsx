...
import { BookOpen, Code, ArrowRight, ArrowLeft, LayoutDashboard, ChevronRight } from "lucide-react";
...
const Subjects = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/subjects/${regNo}`)
      .then(res => res.json())
  ...
  return (
    <div className="subjects-page-container">
      {/* Left Section: Info Section from Image */}
        ...
        {data.subjects.map((sub, i) => (
          ...
          >
            <div className="icon-circle">
              {sub.name.toLowerCase().includes('lab') ? <Code size={20} /> : <BookOpen size={20} />}
            </div>
            
            <div className="card-content">
              <span className="card-subtext">{sub.code}</span>
              <h3>{sub.name}</h3>
              <p>Access study materials, previous papers, and lab manuals for this module.</p>
